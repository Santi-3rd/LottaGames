...
class Backlog_Management(User_permissions):
    ...
    def post(self, request):
        ...
        backlog_game = Backlog(app_user=request.user, game=game_id)
        backlog_game.save()

        new_backlog_game = BacklogSerializer(backlog_game)
        
        return Response(new_backlog_game.data, status=HTTP_201_CREATED)
    
    def get(self, request):
        backlog_games = Backlog.objects.filter(app_user=request.user)
        games_data = BacklogSerializer(backlog_games, many=True).data
        return Response(games_data)
    
    def delete(self, request, game):

        a_backlog_game = get_object_or_404(Backlog, app_user=request.user, game=game)
        a_backlog_game.delete()
